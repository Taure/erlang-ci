...
> **Security:** All release tags are immutable (protected by repository rulesets). Pin to exact semver versions (e.g. `@v1.14.0`). Floating major tags (`@v1`, `@v2`) are not used.
...
jobs:
  ci:
    uses: Taure/erlang-ci/.github/workflows/ci.yml@v1.14.0
    with:
      otp-version: '28'
```
...
jobs:
  ...
  release:
    ...
    uses: Taure/erlang-ci/.github/workflows/release.yml@v1.14.0
    permissions:
      contents: write
    secrets: inherit
```
...
steps:
  ...
  - uses: Taure/erlang-ci@v1.14.0
    with:
      otp-version: '28'
  - run: rebar3 compile
  - uses: Taure/erlang-ci/dialyzer@v1.14.0
    with:
      otp-version: '28'
  - uses: Taure/erlang-ci/elp@v1.14.0
    with:
      mode: both
```
...
steps:
  ...
  - uses: Taure/erlang-ci@v1.14.0
    with:
      otp-version: '28'
  - run: rebar3 compile
  - uses: Taure/erlang-ci/audit@v1.14.0
    id: audit
    with:
      level: high
  - uses: Taure/erlang-ci/sbom@v1.14.0
    with:
      scan: 'true'
...
SBOM scan uses [Trivy](https://github.com/aquasecurity/trivy) against a CycloneDX SBOM generated by `rebar3 sbom`. The `trivy-action` is pinned by SHA to mitigate supply chain attacks (see [trivy-action incident, March 2026](https://github.com/aquasecurity/trivy/discussions/10425)). The build fails on `high` or `critical` severity vulnerabilities. Duplicate matches (same CVE + package from multiple data sources) are deduplicated automatically.
